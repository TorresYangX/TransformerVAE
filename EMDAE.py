...
def retrieval(scoreFile, targetProb_, historicalProb_, targetNum, retrievedTrajectories, historicalTrajectories):
    ...
    for i in range(len(targetProb_)):
        nearest_dist, nearest_ind = tree.query(targetProb_[i].reshape((1,4)), k=targetNum)
...
def cityEMD(targetTrajectories, reterievedTrajectories_, thresholdDistance, latS=39.6, latN=40.2, lonW=116.0, lonE=116.8, NLAT=8, NLON=8):
    ...
    np.save('../small_results/AE/KDTreeAE/EMD/emd_.npy', emd_)
    return 0

if __name__ == '__main__':
    BATCH_SIZE = 16
    path_ = '../small_results/AE/KDTreeAE/EMD/'
    if not os.path.exists(path_):
        os.mkdir(path_)
    historicalData = '../small_data/data_before_time/'
    historicalProb = '../small_results/AE/Index/History/'
    targetData = '../small_data/query_data_before_time/2_17.csv'
    targetProb = '../small_results/AE/Index/Query/prob_2_17.csv'
    queryOutputPath = '../small_results/AE/KDTreeAE/EMD/queryTrajectories.csv'
    scoreFile = '../small_results/AE/KDTreeAE/EMD/meanLoss.csv'
    ...
    retrievedTrajectories = '../small_results/AE/KDTreeAE/EMD/retrievedTrajectories.csv'
