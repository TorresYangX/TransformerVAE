...
NVAE_data = pd.read_csv(NVAE, header=None).to_numpy()
...
def matricbarChart():
    ...
    NVAE_NMD = float(NVAE_data[1][0].split(':')[1])
    ...
    NVAE_NMA = float(NVAE_data[2][0].split(':')[1])
    ...
    NVAE_RRNSA = float(NVAE_data[3][0].split(':')[1])
    ...
    NVAE_SP = float(NVAE_data[4][0].split(':')[1])
    ...
    ## NVAE
    ax.bar(x - 2*width, [NVAE_NMD, NVAE_NMA, NVAE_RRNSA, NVAE_SP], width, label='NVAE')
    ## AE
...
def mdBarChart():
    ...
    NVAE_MD = float(NVAE_data[0][0].split(':')[1])
    ...
    width = 0.2
    fig, ax = plt.subplots()
    ax.bar(x - width/2, [AE_MD, VAE_MD, NVAE_MD, Transformer_MD, LCSS_MD, EDR_MD, EDwP_MD, DTW_MD], width, label='MD')
    ax.set_ylabel('Value')
    ax.set_title('MD')
    ax.set_xticks(x)
    ax.set_xticklabels(['AE', 'VAE', 'NVAE', 'Transformer', 'LCSS', 'EDR', 'EDwP', 'DTW'])
    ax.legend()
