...
def retrieval(scoreFile, targetProb_, historicalProb_, targetNum, retrievedTrajectories, historicalTrajectories):
    ...
    for i in range(len(targetProb_)):
        nearest_dist, nearest_ind = tree.query(targetProb_[i].reshape((1,2*latent_dim)), k=targetNum)
...
def cityEMD(targetTrajectories, reterievedTrajectories_, thresholdDistance, latS=39.6, latN=40.2, lonW=116.0, lonE=116.8, NLAT=8, NLON=8):
    ...
    np.save('../small_results/VAE_transformer_new/KDTreeVAE_transformer/EMD/emd_.npy', emd_)
    return 0
        
if __name__ == '__main__':
    BATCH_SIZE = 16
    path_ = '../small_results/VAE_transformer_new/KDTreeVAE_transformer/EMD/'
    if not os.path.exists(path_):
        os.mkdir(path_)
    historicalData = '../small_data/data_before_time/'
    historicalMu = '../small_results/VAE_transformer_new/Index/History/mu/'
    historicalSigma = '../small_results/VAE_transformer_new/Index/History/sigma/'
    targetData = '../small_data/query_data_before_time/2_17.csv'
    targetMu = '../small_results/VAE_transformer_new/Index/Query/mu/mu_2_17.csv'
    targetSigma = '../small_results/VAE_transformer_new/Index/Query/sigma/sigma_2_17.csv'
    queryOutputPath = '../small_results/VAE_transformer_new/KDTreeVAE_transformer/EMD/queryTrajectories.csv'
    scoreFile = '../small_results/VAE_transformer_new/KDTreeVAE_transformer/EMD/meanLoss.csv'
    ...
    retrievedTrajectories = '../small_results/VAE_transformer_new/KDTreeVAE_transformer/EMD/retrievedTrajectories.csv'
