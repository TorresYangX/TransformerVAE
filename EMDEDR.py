...
def cityEMD(targetTrajectories, reterievedTrajectories_, thresholdDistance, latS=39.6, latN=40.2, lonW=116.0, lonE=116.8, NLAT=8, NLON=8):
    ...
    np.save('../small_results/EDR/KDTreeEDR/EMD/emd_.npy', emd_)
    return 0

if __name__ == '__main__':
    BATCH_SIZE = 16
    history = 6
    path_ = '../small_results/EDR/KDTreeEDR/EMD/'
    if not os.path.exists(path_):
        os.mkdir(path_)
    historicalData = '../small_data/data_before_time/'
    targetData = '../small_data/query_data_before_time/8_7.csv'
    historicalScore_ = '../small_results/EDR/'
    scoreFile = '../small_results/EDR/KDTreeEDR/EMD/meanLoss.csv'.format(history)
    ...
    retrievedTrajectories = '../small_results/EDR/KDTreeEDR/EMD/retrievedTrajectories.csv'.format(history)
